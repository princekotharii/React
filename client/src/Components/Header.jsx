...
function Header() {
  ...
  // Background logic
const navClass = isNavPage
    ? isListHovered || scrolled
    ? 'bg-white text-black'
 ...
 return (
    <nav
        className={`fixed top-0 left-0 w-full flex items-center mx-1 h-[64px] xl:h-[7.3655vw]
        gap-[2.22222222vw] transition-all duration-500 ease-in-out
        p-[0_8px_0_8px] xl:p-[2.2222vw_1.1111vw_0vw_1.1111vw]
        ${navClass} ${showHeader ? 'translate-y-0' : '-translate-y-full'} z-50 `}
    >
       ...
       {/* Right Top Links */}
        ...
        <NavLink to="" className="p-[4px_10px] flex items-center gap-1">For Business <FiTrendingUp /></NavLink>
        </div>
        ...
        <ul
            className='list-none flex text-[17px] font-bold gap-6'
            onMouseEnter={() => setIsListHovered(true)}
            onMouseLeave={() => setIsListHovered(false)}
        >
            <li><NavLink to='/shop'>Shop</NavLink></li>
            <li><NavLink to='/mobile'>Mobile</NavLink></li>
            <li><NavLink to='/av&tv'>AV & TV</NavLink></li>
        ...
        <button
        type='button'
        className={`flex items-center xl:border xl:border-[rgba(255,255,255,0.2)] p-[9px]
        xl:p-[10px_12px] xl:mr-[.69444444vw] gap-[.55555556vw]
        xl:w-[176px] xl:rounded-[6.94444444vw]
        ${isNavPage
            ? (isListHovered || scrolled
            ? "bg-[#f7f7f7] text-black"
            : "xl:bg-[rgba(255,255,255,0.1)] text-white")
            : "bg-[#f7f7f7] text-black"}
        cursor-pointer relative justify-start`}
        >
        <GoSearch className='w-[24px] h-[24px]' />
        <span className='hidden xl:block ml-2'>Search</span>
        </button>
        ...
        {/* User */}
        <button onClick={() => navigate('./login')} className='group p-[9px] xl:p-[9px] items-center cursor-pointer'>
        ...
        <div className='z-10 bg-white text-black absolute hidden xl:group-hover:flex right-[.44444445vw] shadow-[0_4px_10px_0_rgba(0,0,0,.2)] mt-[15px] p-[24px] rounded-[1.38888889vw] w-[18.33333333vw] flex-col text-start'>
            <Link className='font-bold p-[.69444444vw_1.11111111vw_.83333333vw_1.11111111vw] text-[.97222222vw]'>{user || "Login"}</Link>
            <Link className='mb-[.625vw] p-[.69444444vw_1.11111111vw_.83333333vw_1.11111111vw] text-[.97222222vw]'>Why Create a Samsung Account?</Link>
            <Link className='p-[.69444444vw_1.11111111vw_.83333333vw_1.11111111vw] text-[.97222222vw]'>Order</Link>
            <Link className='p-[.69444444vw_1.11111111vw_.83333333vw_1.11111111vw] text-[.97222222vw]'>Product Registration</Link>
            <Link className='p-[.69444444vw_1.11111111vw_.83333333vw_1.11111111vw] text-[.97222222vw]'>Digital Service Center</Link>
        </div>
        </button>
